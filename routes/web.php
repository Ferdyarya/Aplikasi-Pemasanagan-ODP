...
Route::get('/', function () {
    ...
    // Ambil data pemasangan per hari (7 hari terakhir)
    $tanggalPerHari = [];
    $pemasanganPerHari = [];
    $kerusakanPerHari = [];

    for ($i = 6; $i >= 0; $i--) {
        $tanggal = Carbon::today()->subDays($i)->format('d F Y'); // dd-mm-yyyy
        $tanggalPerHari[] = $tanggal;

        $pemasanganPerHari[] = Pemasangan::whereDate('created_at', Carbon::today()->subDays($i))->count();
        $kerusakanPerHari[] = Kerusakan::whereDate('created_at', Carbon::today()->subDays($i))->count();
    }

    return view('dashboard', compact('pemasangancount', 'perbaikancount', 'kerusakancount', 'jaringancount', 'tanggalPerHari', 'pemasanganPerHari', 'kerusakanPerHari'));
})->middleware('auth');
...
Route::prefix('dashboard')
    ...
    ->group(function () {
        ...
        Route::get('laporankerusakan', [KerusakanController::class, 'filterkerusakan'])->name('laporankerusakankategori');
